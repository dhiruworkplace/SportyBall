...
public class ShopPanel : MonoBehaviour
{
    ...
    private List<int> prices = new List<int>() { 0, 10000, 20000, 30000, 40000, 50000 };
    ...
    public void SelectTheme(int index)
    {
        if (PlayerPrefs.GetInt("p" + index, 0) == 1)
        {
            ...
            {
                AppGoals.selectedTheme = index;
                CheckAllTheme();
            }
        }
        else
        {
            if (AppGoals.coins >= prices[index])
            {
                AppGoals.coins -= prices[index];
